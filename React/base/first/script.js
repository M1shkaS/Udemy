...
// У компонента 3 этапа жизненных цикла:
// 1 Компонент появяется на странице (ComponentDidMount())
// 2 Компонент обновился(если компонент получил новое свойство, то он перерисовывается; если внутри компонента изменён stat, то  он тоже обновляется и перерисовывается)(ComponentDidUpdate())
// 3 Компонент становится ненужным и просто удаляется со страницы(ComponentWillUnmount())
// Есть ещё этап  и называется этап ошибки(ComponentDidCatch())